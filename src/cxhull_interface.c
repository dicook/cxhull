...
void qh_produce_output(qhT* qh) {
  ...
  if(qh_setsize(qh, qh->qhmem.tempstack) != tempsize) {
    qh_fprintf(qh, qh->ferr, 6206,
               "qhull internal error (qh_produce_output): temporary sets not "
               "empty(%d)\n",
...
void qh_produce_output2(qhT* qh) {
  ...
  for(i = 0; i < qh_PRINTEND; i++)
    qh_printfacets(qh, qh->fout, qh->PRINTout[i], qh->facet_list, NULL,
                   !qh_ALL);
  qh_allstatistics(qh);
  if(qh->PRINTprecision && !qh->MERGING &&
     (qh->JOGGLEmax > REALmax / 2 || qh->RERUN))
  ...
  if(qh->PRINTstatistics) {
    ...
    size in bytes: merge %d ridge %d vertex %d facet %d\n\
         normal %d ridge vertices %d facet vertices or neighbors %d\n",
               (int)sizeof(mergeT), (int)sizeof(ridgeT), (int)sizeof(vertexT),
               (int)sizeof(facetT), qh->normal_size, d_1, d_1 + SETelemsize);
  }
  if(qh_setsize(qh, qh->qhmem.tempstack) != tempsize) {
    qh_fprintf(qh, qh->ferr, 6065,
               "qhull internal error (qh_produce_output2): temporary sets not "
               "empty(%d)\n",
               qh_setsize(qh, qh->qhmem.tempstack));
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
...
int qh_compare_facetvisit(const void* p1, const void* p2) {
  ...
  if(!(i = a->visitid))
    i = 0 -
        a->id; /* do not convert to int, sign distinguishes id from visitid */
  if(!(j = b->visitid))
    j = 0 - b->id;
  return (i - j);
} /* compare_facetvisit */
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  numtricoplanars  ) count good facets for printing and set visitid if
  allfacets, ignores qh_skipfacet()
...
void qh_countfacets(qhT* qh,
  ...
  facetT *facet, **facetp;
  int numfacets = 0, numsimplicial = 0, numridges = 0, totneighbors = 0,
      numcoplanars = 0, numtricoplanars = 0;
...
pointT* qh_detvnorm(qhT* qh,
  ...
  if(numcenters > dim) {
    ...
    qh_fprintf(qh, qh->ferr, 6216,
               "qhull internal error (qh_detvnorm): too few points(%d) to "
  ...
  qh->gm_row[i] = gmcoord; /* does not overlap midpoint, may be used later for
                              qh_areasimplex */
  normal = gmcoord;
  qh_sethyperplane_gauss(qh, dim, qh->gm_row, point0, True, normal, &offset,
                         &nearzero);
  ...
  if(qh->VERIFYoutput || qh->PRINTstatistics) {
    ...
    if(!unbounded) {
      ...
      trace4((qh, qh->ferr, 4014,
              "qh_detvnorm: points %d %d midpoint dist %2.2g\n", pointid,
              pointidA, dist));
      for(k = 0; k < dim; k++)
        midpoint[k] =
            vertexA->point[k] - vertex->point[k]; /* overwrites midpoint! */
      qh_normalize(qh, midpoint, dim, False);
      angle = qh_distnorm(dim, midpoint, normal,
                          &zero); /* qh_detangle uses dim+1 */
      if(angle < 0.0)
        angle = angle + 1.0;
      else
        angle = angle - 1.0;
      if(angle < 0.0)
        angle -= angle;
      trace4((qh, qh->ferr, 4015,
              "qh_detvnorm: points %d %d angle %2.2g nearzero %d\n", pointid,
              pointidA, angle, nearzero));
      if(nearzero) {
        zzinc_(Zridge0);
        wwmax_(Wridge0max, angle);
        wwadd_(Wridge0, angle);
      } else {
        zzinc_(Zridgeok) wwmax_(Wridgeokmax, angle);
    ...
    if(neighbor->seen) {
      if(neighbor->visitid) {
        if(!neighbor->tricoplanar ||
           qh_setunique(qh, &tricenters, neighbor->center))
...
setT* qh_detvridge3(qhT* qh, vertexT* atvertex, vertexT* vertex) {
  ...
  FOREACHneighbor_(atvertex) neighbor->seen2 = False;
  FOREACHneighbor_(vertex) {
    if(!neighbor->seen2) {
      facet = neighbor;
      break;
  ...
  if(qh->CHECKfrequently) {
    FOREACHneighbor_(vertex) {
      if(!neighbor->seen2) {
        qh_fprintf(qh, qh->ferr, 6217,
                   "qhull internal error (qh_detvridge3): neighbors of vertex "
                   "p%d are not connected at facet %d\n",
                   qh_pointid(qh, vertex->point), neighbor->id);
        qh_errexit(qh, qh_ERRqhull, neighbor, NULL);
      }
    }
  }
  FOREACHneighbor_(atvertex) neighbor->seen2 = True;
  qh_settempfree(qh, &tricenters);
  return centers;
} /* detvridge3 */
        ...
        if(vertex->visitid != qh->vertex_visit && !vertex->seen) {
            ...
            if(neighborA->seen) {
              if(neighborA->visitid) {
                if(!neighborA->tricoplanar ||
                   qh_setunique(qh, &tricenters, neighborA->center))
                  count++;
              } else if(firstinf) {
                count++;
                firstinf = False;
          ...
          if(count >= qh->hull_dim - 1) { /* e.g., 3 for 3-d Voronoi */
            ...
            trace4((qh, qh->ferr, 4017,
                    "qh_eachvoronoi: Voronoi ridge of %d vertices between "
                    "sites %d and %d\n",
...
int qh_eachvoronoi_all(qhT* qh,
  ...
  FORALLvertices {
    if(qh->GOODvertex > 0 &&
      ...
      continue;
    totridges += qh_eachvoronoi(qh, fp, printvridge, vertex, !qh_ALL,
                                innerouter, inorder);
  }
  return totridges;
} /* eachvoronoi_all */
  ...
  if(qh->MERGING || qh->JOGGLEmax < REALmax / 2) {
    ...
    if(qh->JOGGLEmax < REALmax / 2)
      radius -=
          qh->JOGGLEmax *
          sqrt((realT)
                   qh->hull_dim); /* already accounted for in qh_outerinner() */
    *outerplane += radius;
    *innerplane -= radius;
...
void qh_markkeep(qhT* qh, facetT* facetlist) {
  ...
  trace2((qh, qh->ferr, 2006,
          "qh_markkeep: only keep %d largest and/or %d most merged facets "
          "and/or min area %.2g\n",
  ...
  if(qh->KEEParea) {
    qsort(SETaddr_(facets, facetT), (size_t)size, sizeof(facetT*),
...
setT* qh_markvoronoi(qhT* qh,
  ...
  qh->printoutnum++;
  qh_clearcenters(qh,
                  qh_ASvoronoi); /* in case, qh_printvdiagram2 called by user */
  qh_vertexneighbors(qh);
...
void qh_order_vertexneighbors(qhT* qh, vertexT* vertex) {
  ...
  while(qh_setsize(qh, vertex->neighbors)) {
    ...
    if(!neighbor) {
      qh_fprintf(qh, qh->ferr, 6066,
                 "qhull internal error (qh_order_vertexneighbors): no neighbor "
                 "of v%d for f%d\n",
                 vertex->id, facet->id);
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  qh_prepare_output(qh, )
    prepare for qh_produce_output2(qh) according to
      qh.KEEPminArea, KEEParea, KEEPmerge, GOODvertex, GOODthreshold, GOODpoint,
  ONLYgood, SPLITthresholds does not reset facet->good
  ...
  switch(format) {
    ...
    case qh_PRINTtriangles:
      if(qh->hull_dim == 3 && format != qh_PRINTtriangles)
        ...
        qh_printfacetNvertex_nonsimplicial(qh, fp, facet, qh->printoutvar++,
                                           format);
      break;
    case qh_PRINTinner:
    ...
    case qh_PRINTneighbors:
      ...
      FOREACHneighbor_(facet) qh_fprintf(
          qh, fp, 9024, " %d",
    ...
    case qh_PRINTpointintersect:
      if(!qh->feasible_point) {
        qh_fprintf(qh, qh->ferr, 6067,
                   "qhull input error (qh_printafacet): option 'Fp' needs "
                   "qh->feasible_point\n");
        qh_errexit(qh, qh_ERRinput, NULL, NULL);
      }
      if(facet->offset > 0)
      ...
      if(facet->offset < -qh->MINdenom) {
        ...
        for(k = qh->hull_dim; k--;) {
          *(coordp++) =
              qh_divzero(*(normp++), facet->offset, qh->MINdenom_1, &zerodiv) +
              *(feasiblep++);
          if(zerodiv) {
  ...
  switch(format) {
    ...
    case qh_PRINTgeom:
      if(qh->hull_dim > 4) /* qh_initqhull_globals also checks */
        goto LABELnoformat;
      if(qh->VORONOI &&
         qh->hull_dim > 3) /* PRINTdim == DROPdim == hull_dim-1 */
        goto LABELnoformat;
      if(qh->hull_dim == 2 && (qh->PRINTridges || qh->DOintersections))
        qh_fprintf(qh, qh->ferr, 7049,
                   "qhull warning: output for ridges and intersections not "
                   "implemented in 2-d\n");
      if(qh->hull_dim == 4 && (qh->PRINTinner || qh->PRINTouter ||
                               (qh->PRINTdim == 4 && qh->PRINTcentrums)))
        qh_fprintf(qh, qh->ferr, 7050,
                   "qhull warning: output for outer/inner planes and centrums "
                   "not implemented in 4-d\n");
      if(qh->PRINTdim == 4 && (qh->PRINTspheres))
        qh_fprintf(
            qh, qh->ferr, 7051,
            "qhull warning: output for vertices not implemented in 4-d\n");
      if(qh->PRINTdim == 4 && qh->DOintersections && qh->PRINTnoplanes)
        qh_fprintf(qh, qh->ferr, 7052,
                   "qhull warning: 'Gnh' generates no output in 4-d\n");
      if(qh->PRINTdim == 2) {
        qh_fprintf(qh, fp, 9036, "{appearance {linewidth 3} LIST # %s | %s\n",
                   qh->rbox_command, qh->qhull_command);
      } else if(qh->PRINTdim == 3) {
        qh_fprintf(qh, fp, 9037,
        ...
        FOREACHfacet_(facets) qh_printend4geom(qh, NULL, facet, &num, printall);
        qh->ridgeoutnum = num;
      ...
      if(qh->PRINTdots) {
        ...
        FORALLpoints {
          if(!qh->DELAUNAY || !qh->ATinfinity ||
             qh_pointid(qh, point) != qh->num_points - 1) {
            if(qh->PRINTdim == 4)
              qh_printpoint(qh, fp, NULL, point);
            else
      ...
      } else if(!qh->PREmerge && qh->POSTmerge &&
                qh->postmerge_cos < REALmax / 2) {
        ...
        if(qh->PRINTcoplanar || qh->PRINTcentrums) {
          ...
          if(qh->PRINTcoplanar && !qh->PRINTspheres) {
            FOREACHvertex_(vertices)
                qh_printpointvect2(qh, fp, vertex->point, NULL,
          ...
          FORALLfacet_(facetlist) {
            ...
            if(!qh->PRINTcoplanar)
              continue;
            FOREACHpoint_(facet->coplanarset) qh_printpointvect2(
                qh, fp, point, facet->normal, NULL, qh->PRINTradius);
            FOREACHpoint_(facet->outsideset) qh_printpointvect2(
                qh, fp, point, facet->normal, NULL, qh->PRINTradius);
          }
          FOREACHfacet_(facets) {
            if(!printall && qh_skipfacet(qh, facet))
    ...
    case qh_PRINTincidences:
      if(qh->VORONOI && qh->PRINTprecision)
        qh_fprintf(qh, qh->ferr, 7053,
                   "qhull warning: writing Delaunay.  Use 'p' or 'o' for "
                   "Voronoi centers\n");
      qh->printoutvar =
          qh->vertex_id; /* centrum id for non-simplicial facets */
      if(qh->hull_dim <= 3)
    ...
    case qh_PRINTouter:
      if(qh->CDDoutput)
        qh_fprintf(qh, fp, 9052, "%s | %s\nbegin\n    %d %d real\n",
                   qh->rbox_command, qh->qhull_command, numfacets,
                   qh->hull_dim + 1);
      else
    ...
    case qh_PRINTtriangles:
      ...
      if(format == qh_PRINToff || qh->hull_dim == 2)
        qh_fprintf(qh, fp, 9060, "%d\n%d %d %d\n", num,
                   qh->num_points + qh_setsize(qh, qh->other_points), numfacets,
                   totneighbors / 2);
      else { /* qh_PRINTtriangles */
        qh->printoutvar = qh->num_points +
                          qh_setsize(qh, qh->other_points); /* first centrum */
        if(qh->DELAUNAY)
          num--; /* drop last dimension */
        qh_fprintf(qh, fp, 9061, "%d\n%d %d %d\n", num,
                   qh->printoutvar + numfacets - numsimplicial,
                   numsimplicial + numridges, totneighbors / 2);
      }
      FORALLpoints qh_printpointid(qh, qh->fout, NULL, num, point,
                                   qh_IDunknown);
      FOREACHpoint_(qh->other_points)
    ...
    default:
    LABELnoformat:
      qh_fprintf(qh, qh->ferr, 6068,
                 "qhull internal error (qh_printbegin): can not use this "
                 "format for dimension %d\n",
                 qh->hull_dim);
      qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
} /* printbegin */
  ...
  switch(format) {
    case qh_PRINTgeom:
      if(qh->hull_dim == 4 && qh->DROPdim < 0 && !qh->PRINTnoplanes) {
        qh->visit_id++;
        num = 0;
        FORALLfacet_(facetlist) qh_printend4geom(qh, fp, facet, &num, printall);
        FOREACHfacet_(facets) qh_printend4geom(qh, fp, facet, &num, printall);
        if(num != qh->ridgeoutnum || qh->printoutvar != qh->ridgeoutnum) {
          qh_fprintf(qh, qh->ferr, 6069,
                     "qhull internal error (qh_printend): number of ridges %d "
                     "!= number printed %d and at end %d\n",
...
void qh_printend4geom(qhT* qh,
  ...
  if(facet->simplicial) {
      ...
      if(neighbor->visitid != qh->visit_id) {
        if(fp)
          qh_fprintf(qh, fp, 9084, "3 %d %d %d %8.4g %8.4g %8.4g 1 # f%d f%d\n",
                     3 * num, 3 * num + 1, 3 * num + 2, color[0], color[1],
                     color[2], facet->id, neighbor->id);
        num++;
      }
    }
  } else {
    FOREACHridge_(facet->ridges) {
      ...
      if(neighbor->visitid != qh->visit_id) {
        if(fp)
          qh_fprintf(qh, fp, 9085,
                     "3 %d %d %d %8.4g %8.4g %8.4g 1 #r%d f%d f%d\n", 3 * num,
                     3 * num + 1, 3 * num + 2, color[0], color[1], color[2],
...
void qh_printextremes_2d(qhT* qh,
  ...
  qh_countfacets(qh, facetlist, facets, printall, &numfacets, &numsimplicial,
                 ...
                 &numtricoplanars); /* marks qh->visit_id */
  vertices = qh_facetvertices(qh, facetlist, facets, printall);
  ...
  do {
    ...
    if(facet->visitid == qh->visit_id) {
      qh_fprintf(qh, qh->ferr, 6218,
                 "Qhull internal error (qh_printextremes_2d): loop in facet "
                 "list.  facet %d nextfacet %d\n",
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  qh_printfacet2geom(qh, fp, facet, color )
    ...
    notes:
      assume precise calculations in io_r.c with roundoff covered by
  qh_GEOMepsilon mindist is calculated within io_r.c.  maxoutside is calculated
  elsewhere so a DISTround error may have occurred.
*/
void qh_printfacet2geom(qhT* qh, FILE* fp, facetT* facet, realT color[3]) {
  ...
  if(qh->PRINTinner ||
     (!qh->PRINTnoplanes && !qh->PRINTouter &&
...
void qh_printfacet2math(qhT* qh,
  ...
  qh_fprintf(qh, fp, 9097, pointfmt, point0[0], point0[1], point1[0],
             point1[1]);
  qh_memfree(qh, point1, qh->normal_size);
  qh_memfree(qh, point0, qh->normal_size);
} /* printfacet2math */
...
/*-<a                             href="qh-io_r.htm#TOC"
  >-------------------------------</a><a
  name="printfacet3geom_nonsimplicial">-</a>
...
void qh_printfacet3geom_nonsimplicial(qhT* qh,
  ...
  if(qh->PRINTouter || (!qh->PRINTnoplanes && !qh->PRINTinner))
    qh_printfacet3geom_points(qh, fp, projectedpoints, facet, outerplane,
                              color);
  if(qh->PRINTinner ||
     (!qh->PRINTnoplanes && !qh->PRINTouter &&
      outerplane - innerplane > 2 * qh->MAXabs_coord * qh_GEOMepsilon)) {
    for(k = 3; k--;)
      color[k] = 1.0 - color[k];
    qh_printfacet3geom_points(qh, fp, projectedpoints, facet, innerplane,
                              color);
  }
  FOREACHpoint_(projectedpoints) qh_memfree(qh, point, qh->normal_size);
    ...
    FOREACHridge_(facet->ridges) {
      ...
      if(neighbor->visitid != qh->visit_id) {
        if(qh->DOintersections)
          qh_printhyperplaneintersection(qh, fp, facet, neighbor,
                                         ridge->vertices, black);
        if(qh->PRINTridges) {
...
void qh_printfacet3geom_points(qhT* qh,
  ...
  if(offset != 0.0) {
    ...
    FOREACHpoint_(points) qh_setappend(
        qh, &printpoints, qh_projectpoint(qh, point, facet, -offset));
  } else
    printpoints = points;
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  notes:
    ...
    assume precise calculations in io_r.c with roundoff covered by
  qh_GEOMepsilon innerplane may be off by qh->DISTround.  Maxoutside is
  calculated elsewhere so a DISTround error may have occurred.
*/
void qh_printfacet3geom_simplicial(qhT* qh,
  ...
  FOREACHvertex_(vertices) qh_setappend(qh, &points, vertex->point);
  if(qh->PRINTouter || (!qh->PRINTnoplanes && !qh->PRINTinner))
  ...
  if((qh->DOintersections || qh->PRINTridges) &&
    ...
    FOREACHneighbor_(facet) {
      if(neighbor->visitid != qh->visit_id) {
        vertices =
            qh_setnew_delnthsorted(qh, facet->vertices, qh->hull_dim,
                                   SETindex_(facet->neighbors, neighbor), 0);
        if(qh->DOintersections)
          qh_printhyperplaneintersection(qh, fp, facet, neighbor, vertices,
                                         black);
        if(qh->PRINTridges) {
...
void qh_printfacet3math(qhT* qh,
  ...
  FOREACHpoint_(points) qh_memfree(qh, point, qh->normal_size);
  qh_settempfree(qh, &points);
  qh_settempfree(qh, &vertices);
...
/*-<a                             href="qh-io_r.htm#TOC"
  >-------------------------------</a><a
  name="printfacet4geom_nonsimplicial">-</a>
...
void qh_printfacet4geom_nonsimplicial(qhT* qh,
  ...
  FOREACHridge_(facet->ridges) {
    ...
    if(qh->PRINTtransparent && !neighbor->good)
      continue;
    if(qh->DOintersections)
      qh_printhyperplaneintersection(qh, fp, facet, neighbor, ridge->vertices,
                                     color);
    else {
      if(qh->DROPdim >= 0)
...
/*-<a                             href="qh-io_r.htm#TOC"
  >-------------------------------</a><a
  name="printfacetNvertex_nonsimplicial">-</a>
...
/*-<a                             href="qh-io_r.htm#TOC"
  >-------------------------------</a><a
  name="printfacetNvertex_simplicial">-</a>
...
void qh_printfacetheader(qhT* qh, FILE* fp, facetT* facet) {
  ...
  qh_printpointid(qh, fp, "    - normal: ", qh->hull_dim, facet->normal,
                  qh_IDunknown);
  ...
  if(!SETempty_(facet->outsideset)) {
    ...
    if(qh_setsize(qh, facet->outsideset) < 6) {
      ...
      FOREACHpoint_(facet->outsideset) qh_printpoint(qh, fp, "     ", point);
    } else if(qh_setsize(qh, facet->outsideset) < 21) {
      qh_printpoints(qh, fp, "    - outside set:", facet->outsideset);
    } else {
      qh_fprintf(qh, fp, 9169, "    - outside set:  %d points.",
                 qh_setsize(qh, facet->outsideset));
  ...
  if(!SETempty_(facet->coplanarset)) {
    ...
    if(qh_setsize(qh, facet->coplanarset) < 6) {
      qh_fprintf(qh, fp, 9171, "    - coplanar set(furthest p%d):\n",
                 qh_pointid(qh, furthest));
      FOREACHpoint_(facet->coplanarset) qh_printpoint(qh, fp, "     ", point);
    } else if(qh_setsize(qh, facet->coplanarset) < 21) {
      qh_printpoints(qh, fp, "    - coplanar set:", facet->coplanarset);
    } else {
      qh_fprintf(qh, fp, 9172, "    - coplanar set:  %d points.",
                 qh_setsize(qh, facet->coplanarset));
      qh_printpoint(qh, fp, "  Furthest", furthest);
    }
    zinc_(Zdistio);
...
void qh_printfacetridges(qhT* qh, FILE* fp, facetT* facet) {
  ...
  if(facet->visible && qh->NEWfacets) {
    ...
    FOREACHridge_(facet->ridges) qh_fprintf(qh, fp, 9180, " r%d", ridge->id);
    qh_fprintf(qh, fp, 9181, "\n");
  } else {
    qh_fprintf(qh, fp, 9182, "    - ridges:\n");
    FOREACHridge_(facet->ridges) ridge->seen = False;
    if(qh->hull_dim == 3) {
    ...
    if(numridges != qh_setsize(qh, facet->ridges)) {
      ...
      FOREACHridge_(facet->ridges) qh_fprintf(qh, fp, 9184, " r%d", ridge->id);
...
void qh_printfacets(qhT* qh,
  ...
  qh->RANDOMdist = False;
  if(qh->CDDoutput &&
     (format == qh_PRINTcentrums || format == qh_PRINTpointintersect ||
      format == qh_PRINToff))
    qh_fprintf(qh, qh->ferr, 7056,
               "qhull warning: CDD format is not available for centrums, "
  ...
  if(format == qh_PRINTnone)
    ...
    qh_fprintf(qh, fp, 9193, "10 %d %d %d %d %d %d %d %d %d %d\n2 ",
               qh->hull_dim, qh->num_points + qh_setsize(qh, qh->other_points),
  ...
  } else if(qh->VORONOI && (format == qh_PRINTvertices ||
                            format == qh_PRINTinner || format == qh_PRINTouter))
    qh_printvdiagram(qh, fp, format, facetlist, facets, printall);
  else {
    qh_printbegin(qh, fp, format, facetlist, facets, printall);
    FORALLfacet_(facetlist) qh_printafacet(qh, fp, format, facet, printall);
    FOREACHfacet_(facets) qh_printafacet(qh, fp, format, facet, printall);
    qh_printend(qh, fp, format, facetlist, facets, printall);
  }
  qh->RANDOMdist = qh->old_randomdist;
} /* printfacets */
...
/*-<a                             href="qh-io_r.htm#TOC"
  >-------------------------------</a><a
  name="printhyperplaneintersection">-</a>

  qh_printhyperplaneintersection(qh, fp, facet1, facet2, vertices, color )
    print Geomview OFF or 4OFF for the intersection of two hyperplanes in 3-d or
  4-d
*/
void qh_printhyperplaneintersection(qhT* qh,
  ...
  FOREACHvertex_(vertices) {
    ...
    s = qh_divzero(-dist1 + costheta * dist2, denominator, mindenom,
                   &nearzero1);
    t = qh_divzero(-dist2 + costheta * dist1, denominator, mindenom,
                   &nearzero2);
    ...
    if(qh->PRINTdim <= 3) {
      ...
      qh_fprintf(qh, fp, 9199, "%8.4g %8.4g %8.4g %8.4g # ", p[0], p[1], p[2],
                 p[3]);
    if(nearzero1 + nearzero2)
...
void qh_printline3geom(qhT* qh,
  ...
  if((fabs(pA[0] - pB[0]) > 1e-3) || (fabs(pA[1] - pB[1]) > 1e-3) ||
     (fabs(pA[2] - pB[2]) > 1e-3)) {
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  qh_printpoints_out(qh, fp, facetlist, facets, printall )
    prints vertices, coplanar/inside points, for facets by their point
  coordinates allows qh.CDDoutput
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  qh_printpointvect(qh, fp, point, normal, center, radius, color )
    prints a 2-d, 3-d, or 4-d point as 3-d VECT's relative to normal or to
  center point
*/
...
void qh_printvdiagram(qhT* qh,
  ...
  totcount = qh_printvdiagram2(qh, fp, printvridge, vertices, innerouter,
                               True /* inorder*/);
  qh_settempfree(qh, &vertices);
  ...
  FOREACHvertex_i_(qh, vertices) {
    if(vertex) {
      if(qh->GOODvertex > 0 &&
         qh_pointid(qh, vertex->point) + 1 != qh->GOODvertex)
        continue;
      totcount += qh_eachvoronoi(qh, fp, printvridge, vertex, !qh_ALL,
                                 innerouter, inorder);
    }
  }
  return totcount;
...
void qh_printvertex(qhT* qh, FILE* fp, vertexT* vertex) {
  ...
  qh_fprintf(qh, fp, 9235, "- p%d(v%d):", qh_pointid(qh, vertex->point),
             vertex->id);
  point = vertex->point;
  if(point) {
    for(k = qh->hull_dim; k--;) {
...
void qh_printvertexlist(qhT* qh,
  ...
  FOREACHvertex_(vertices) qh_printvertex(qh, fp, vertex);
  qh_settempfree(qh, &vertices);
} /* printvertexlist */
...
void qh_printvertices(qhT* qh, FILE* fp, const char* string, setT* vertices) {
  ...
  FOREACHvertex_(vertices) qh_fprintf(
      qh, fp, 9246, " p%d(v%d)", qh_pointid(qh, vertex->point), vertex->id);
...
void qh_printvneighbors(qhT* qh,
  ...
  qh_countfacets(qh, facetlist, facets, printall, &numfacets, &numsimplicial,
                 &totneighbors, &numridges, &numcoplanars,
                 &numtricoplanars); /* sets facet->visitid */
    ...
    if(vertex) {
      ...
      if(qh->hull_dim == 3)
        ...
        qsort(SETaddr_(vertex->neighbors, facetT), (size_t)numneighbors,
              sizeof(facetT*), qh_compare_facetvisit);
      FOREACHneighbor_(vertex) qh_fprintf(
          qh, fp, 9250, " %d",
          neighbor->visitid ? neighbor->visitid - 1 : 0 - neighbor->id);
...
void qh_printvoronoi(qhT* qh,
                     ...
                     setT* facets,
                     boolT printall) {
  int k, numcenters, numvertices = 0, numneighbors, numinf, vid = 1, vertex_i,
                     vertex_n;
  facetT *facet, **facetp, *neighbor, **neighborp;
  setT* vertices;
  vertexT* vertex;
    ...
    if(vertex) {
      if(qh->hull_dim == 3)
        ...
        qsort(SETaddr_(vertex->neighbors, facetT),
              (size_t)qh_setsize(qh, vertex->neighbors), sizeof(facetT*),
...
void qh_printvnorm(qhT* qh,
  ...
  qh_fprintf(qh, fp, 9271, "%d %d %d ", 2 + qh->hull_dim,
...
void qh_printvridge(qhT* qh,
  ...
  qh_fprintf(qh, fp, 9275, "%d %d %d", qh_setsize(qh, centers) + 2,
             qh_pointid(qh, vertex->point), qh_pointid(qh, vertexA->point));
  FOREACHfacet_(centers) qh_fprintf(qh, fp, 9276, " %d", facet->visitid);
...
int qh_readfeasible(qhT* qh, int dim, const char* curline) {
  ...
  if(!qh->HALFspace) {
    qh_fprintf(qh, qh->ferr, 6070,
               "qhull input error: feasible point(dim 1 coords) is only valid "
               "for halfspace intersection\n");
    qh_errexit(qh, qh_ERRinput, NULL, NULL);
  }
  if(qh->feasible_string)
    qh_fprintf(qh, qh->ferr, 7057,
               "qhull input warning: feasible point(dim 1 coords) overrides "
               "'Hn,n,n' feasible point for halfspace intersection\n");
  if(!(qh->feasible_point = (coordT*)qh_malloc(dim * sizeof(coordT)))) {
    qh_fprintf(qh, qh->ferr, 6071,
    ...
    while(*s) {
      ...
      *(coords++) = value;
      if(++tokcount == dim) {
        ...
        qh_strtod(s, &t);
        if(s != t) {
          qh_fprintf(qh, qh->ferr, 6072,
                     "qhull input error: coordinates for feasible point do not "
                     "finish out the line: %s\n",
                     s);
          qh_errexit(qh, qh_ERRinput, NULL, NULL);
        }
        return linecount;
  ...
  qh_fprintf(qh, qh->ferr, 6073,
             "qhull input error: only %d coordinates.  Could not read %d-d "
             "feasible point.\n",
             tokcount, dim);
  qh_errexit(qh, qh_ERRinput, NULL, NULL);
  return 0;
...
coordT* qh_readpoints(qhT* qh,
    ...
    while((s = fgets(firstline, qh_MAXfirst, qh->fin))) {
      ...
      if(qh->HALFspace && linecount == 1 && isdigit(*s)) {
        ...
        if(qh_strtol(s, &s) == 1)
          ...
          dimfeasible = 0;
      } else if(!memcmp(firstline, "begin", (size_t)5) ||
                !memcmp(firstline, "BEGIN", (size_t)5))
        break;
  ...
  if(!s) {
    qh_fprintf(qh, qh->ferr, 6075,
               "qhull input error: short input file.  Did not find dimension "
  ...
  if(isdelaunay) {
    ...
    *numpoints = numinput;
    if(diminput < 3) {
      qh_fprintf(qh, qh->ferr, 6221,
                 "qhull input error: dimension %d(first number, includes "
                 "offset) should be at least 3 for halfspaces\n",
                 diminput);
      qh_errexit(qh, qh_ERRinput, NULL, NULL);
    }
    if(dimfeasible) {
      if(dimfeasible != *dimension) {
        qh_fprintf(qh, qh->ferr, 6222,
                   "qhull input error: dimension %d of feasible point is not "
                   "one less than dimension %d for halfspaces\n",
                   dimfeasible, diminput);
        qh_errexit(qh, qh_ERRinput, NULL, NULL);
      }
    } else
  ...
  qh->normal_size =
      *dimension * sizeof(coordT); /* for tracing with qh_printpoint */
  if(qh->HALFspace) {
    qh->half_space = coordp =
        (coordT*)qh_malloc(qh->normal_size + sizeof(coordT));
    if(qh->CDDinput) {
      ...
      if(*s == 'e' || *s == 'E') {
        if(!memcmp(s, "end", (size_t)3) || !memcmp(s, "END", (size_t)3)) {
          if(qh->CDDinput)
            ...
            qh_fprintf(qh, qh->ferr, 7058,
                       "qhull input warning: the input appears to be in cdd "
                       "format.  If so, use 'Fd'\n");
        }
      }
    }
      ...
      if(qh->HALFspace) {
        ...
        *(coords++) = value;
        if(qh->CDDinput && !coordcount) {
          if(value != 1.0) {
            qh_fprintf(qh, qh->ferr, 6077,
                       "qhull input error: for cdd format, point at line %d "
      ...
      if(++coordcount == diminput) {
        ...
        if(isdelaunay) {
          *(coords++) = paraboloid;
          ...
          if(!qh_sethalfspace(qh, *dimension, coords, &coords, normalp, offsetp,
                              qh->feasible_point)) {
            qh_fprintf(qh, qh->ferr, 8048, "The halfspace was on line %d\n",
                       linecount);
            if(wasbegin)
              qh_fprintf(qh, qh->ferr, 8049,
                         "The input appears to be in cdd format.  If so, you "
                         "should use option 'Fd'\n");
            qh_errexit(qh, qh_ERRinput, NULL, NULL);
          }
